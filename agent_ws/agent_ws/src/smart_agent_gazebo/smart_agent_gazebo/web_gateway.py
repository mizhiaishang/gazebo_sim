...
class WebGatewayNode(Node):
    def __init__(self) -> None:
        ...
        self.record_output_dir = os.path.expanduser(
            self.declare_parameter("record_output_dir", "~/agent_records").get_parameter_value().string_value
        )
        self.record_save_interval_sec = float(
            self.declare_parameter("record_save_interval_sec", 2.0).get_parameter_value().double_value
        )
    ...
    def start_recording(self) -> Dict[str, object]:
        with self._record_lock:
            ...
            cmd = [
                ...
                "-p",
                "allow_missing_lidar:=false",
            ]
            try:
                os.makedirs(self.record_output_dir, exist_ok=True)
                self._recorder_proc = subprocess.Popen(cmd)
                # Detect quick-fail cases (missing module/topic bridge/ROS env) to avoid false "started" status.
                time.sleep(0.25)
                if not self._recorder_running_unlocked():
                    exit_code = self._recorder_proc.returncode if self._recorder_proc is not None else -1
                    self._recorder_proc = None
                    message = (
                        f"Recorder exited immediately (code={exit_code}). "
                        "Please verify ROS environment and sensor topic bridge configuration."
                    )
...
def _find_available_port(host: str, preferred_port: int, max_tries: int = 50) -> Optional[int]:
...
def main(args=None) -> None:
    ...
    try:
        ...
        run_port = _find_available_port(node.host, node.port)
        if run_port is None:
            node.get_logger().error(
                f"Failed to find a free TCP port near configured port {node.port} on host {node.host}."
            )
            return
        if run_port != node.port:
            node.get_logger().warn(
                f"Configured port {node.port} is busy, falling back to available port {run_port}."
            )
        uvicorn.run(app, host=node.host, port=run_port, log_level="info")
