...
class DataRecorder(Node):
    def __init__(self) -> None:
        ...
        self.imu_save_hz = float(
            self.declare_parameter("imu_save_hz", 100.0).get_parameter_value().double_value
        )
        if self.imu_save_hz <= 0.0:
            self.imu_save_hz = 100.0
    ...
    def _create_subscribers(self) -> None:
        ...
        self.create_subscription(LaserScan, "/agent/lidar/scan", self._lidar_scan_cb, 20)
    ...
    def _save_aligned_snapshot(self) -> None:
        ...
        if lidar_available and lidar_kind is not None:
            ...
            lidar_meta = {
                "available": False,
                "reason": "scan/points unavailable in tolerance window",
            }
